...
BIN=interchain-securityd

CHAIN_ID="interchain-security-provider"
...
jq '.app_state.gov.voting_params.voting_period = "60s"' /validator$STARTING_VALIDATOR/config/genesis.json > /edited-genesis.json
...
do
    ...
    # and allow local pex
    $BIN gentx --home /validator$i --keyring-backend test --moniker validator$i --chain-id=$CHAIN_ID --ip 7.7.7.$i validator$i 500000000stake
